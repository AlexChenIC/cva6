...
class VerilatorTestHarnessCompTest(unittest.TestCase):
    ...
    def test_obi_target_is_rejected_before_cleaning_the_build(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            make_target(root)
            (root / "config/target/cv32a60x/testbench_cfg.yml").write_text(
                "hier: obi\n"
            )
            old_binary = RECIPE.testharness_binary(root, "cv32a60x", CompMode.rtl)
            old_binary.parent.mkdir(parents=True)
            old_binary.write_text("existing output\n")
            with working_directory(root):
                result = CliRunner().invoke(RECIPE.app, ["-t", "cv32a60x", "--quiet"])
            self.assertEqual(result.exit_code, 1)
            self.assertIn("requires an AXI target", result.output)
            self.assertEqual(old_binary.read_text(), "existing output\n")

    def test_testharness_filelist_sources_exist(self) -> None:
        if not (REPO_ROOT / "corev_apu/riscv-dbg/.git").exists():
            self.skipTest("Filelist existence check requires initialized submodules")
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                patch.object(
                    RECIPE,
                    "_verilator_root",
                    return_value=(install / "share/verilator").resolve(),
                ),
                patch.dict(
                    os.environ,
                    {
    ...
    def test_path_verilator_reports_the_matching_include_root(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            completed = Mock(stdout=f"{verilator_root}\n")
            with (
        ...
        self.assertEqual(
            run.call_args.args[0], ["/usr/bin/verilator", "--getenv", "VERILATOR_ROOT"]
        )
        self.assertNotIn("VERILATOR_ROOT", run.call_args.kwargs["env"])
    ...
    def test_compile_writes_the_canonical_cook_manifest(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            def run_compile(*args, **kwargs):
                ...
                binary.touch()
                binary.chmod(0o755)
                kwargs["log"].write_text("compiled\n", encoding="utf-8")
                return 0, False
            ...
            with (
                ...
                patch.object(RECIPE, "run_logged_process", side_effect=run_compile),
                patch.object(
                    RECIPE.subprocess,
                    "run",
                    return_value=Mock(stdout="Verilator 5.050\n"),
                ),
    ...
    def test_compile_failure_returns_nonzero(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                ...
                patch.object(
                    RECIPE, "run_logged_process", side_effect=RuntimeError("failed")
                ),
                patch.object(
                    RECIPE.subprocess,
                    "run",
    ...
    def test_quiet_validation_failure_is_visible(self) -> None:
        result = CliRunner().invoke(RECIPE.app, ["-t", "../outside", "--quiet"])
        self.assertEqual(result.exit_code, 1)
        self.assertIn("Invalid target name", result.output)

    def test_help_states_the_supported_scope(self) -> None:
        ...
        for failure in ("returncode", "timeout", "binary", "manifest"):
            with (
                ...
                def compile_tool(*args, **kwargs):
                    ...
                    if failure != "binary":
                        binary = RECIPE.testharness_binary(
                            root, "cv32a60x", CompMode.rtl
                        )
                        binary.touch()
                        binary.chmod(0o755)
                    return (1 if failure == "returncode" else 0), failure == "timeout"
                ...
                with (
                    ...
                    patch.object(
                        RECIPE,
                        "tool_paths",
                        return_value=(root, root, "/verilator", root),
                    ),
                    patch.object(
                        RECIPE.subprocess,
                        "run",
                    ...
                    patch.object(
                        RECIPE, "run_logged_process", side_effect=compile_tool
                    ),
                    patch.object(RECIPE, "write_manifest"),
                ):
                    result = CliRunner().invoke(
                        RECIPE.app, ["-t", "cv32a60x", "--quiet"]
                    )
                self.assertEqual(result.exit_code, 1, result.output)
                self.assertTrue(result.output.strip())
                if failure in {"returncode", "timeout"}:
                    self.assertIn("diagnostic fixture", result.output)

    def test_verilator_query_timeout_has_an_actionable_error(self) -> None:
        with patch.object(
            RECIPE.subprocess,
            "run",
            side_effect=RECIPE.subprocess.TimeoutExpired("verilator", 10),
        ):
            with self.assertRaisesRegex(
                ValueError, "Cannot query Verilator installation"
            ):
                RECIPE._verilator_root("/verilator")
