...
class VerilatorTestHarnessRunTest(unittest.TestCase):
    ...
    def test_spike_yaml_is_required_only_for_iss_comparison(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            self.assertEqual(RECIPE.target_configuration(root, False), (None, "msu"))
            with self.assertRaisesRegex(ValueError, "ISS comparison requires"):
                RECIPE.target_configuration(root, True)
            cmd = RECIPE.testharness_command(
                Path("/binary"),
                Path("/program.elf"),
                target="rv64",
                spike_yaml=None,
            ...
            self.assertFalse(any(arg.startswith("+config_file=") for arg in cmd))

    def test_missing_and_malformed_manifests_fail_closed(self):
        for value in (None, "[]\n", "options: []\n", "recipe: other\noptions: {}\n"):
            with self.subTest(value=value), tempfile.TemporaryDirectory() as directory:
                root = Path(directory)
                compile_dir, elab_dir = prepare_run_tree(root)
                path = elab_dir / MANIFEST_NAME
                if value is None:
                    path.unlink()
                else:
                    path.write_text(value)
                with self.assertRaises((ValueError, RECIPE.typer.Exit)):
                    RECIPE.check_manifests(
                        target="cv32a60x",
                        test_name="hello-world",
                        comp_mode=CompMode.rtl,
    ...
    def test_trace_format_must_match_the_compiled_binary(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            compile_dir, elab_dir = prepare_run_tree(root)
            for mode in (TraceMode.fast, TraceMode.compact):
                with self.assertRaises(RECIPE.typer.Exit):
                    RECIPE.check_manifests(
                        target="cv32a60x",
                        test_name="hello-world",
    ...
    def test_existing_trace_parsers_compare_real_register_updates(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            path_before = sys.path.copy()
            (root / "verilator.log").write_text(
                " 4 | core 0: 0x0000000080000000 (0x00100093) li ra,1\n"
                "3 0x0000000080000000 (0x00100093) x1 0x0000000000000001\n"
            )
            spike = root / "spike.log"
            for value, expected in (("1", True), ("2", False)):
                spike.write_text(
                    "core 0: 0x0000000080000000 (0x00100093) li ra,1\n"
                    f"core 0: 3 0x0000000080000000 (0x00100093) x1 0x000000000000000{value}\n"
                )
    ...
    def test_run_result_records_failure_without_warning_policy(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            log = root / "testharness.log"
            log.write_text("UVM_WARNING spike_tandem\n*** SUCCESS *** (tohost = 0)\n")
            self.assertTrue(RECIPE.testharness_log_passed(log)[0])
            with patch.object(
                RECIPE, "run_test", return_value=(False, "comparison failed", root)
            ):
                result = CliRunner().invoke(
                    RECIPE.app, ["-t", "cv32a60x", "-n", "hello", "--quiet"]
                )
            self.assertEqual(result.exit_code, 1)
            self.assertIn("comparison failed", result.output)
            data = yaml.safe_load((root / "result.yml").read_text())
            self.assertEqual(data["status"], "FAIL")

    def test_zero_tohost_fails_before_simulation_cleanup(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            compile_dir, _ = prepare_run_tree(root)
            (compile_dir / "hello-world.add_tohost").write_text("00000000\n")
            with working_directory(root), self.assertRaisesRegex(ValueError, "tohost"):
                RECIPE.run_test(
                    target="cv32a60x",
                    test_name="hello-world",
                    comp_mode=CompMode.rtl,
