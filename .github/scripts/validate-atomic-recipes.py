"""Optional native conformance checks; no legacy simulator orchestration."""
...
paths = [
    ...
    "flows/utils/logged_process.py",
    "verif/tb/core/Flist.verilator_testharness",
]
provenance = {
    ...
    "suite": suite,
    "source_blobs": {
        p: subprocess.check_output(["git", "hash-object", p], text=True).strip()
        for p in paths
    },
...
def run(command):
    ...
    with (out / f"{index:02d}.log").open("w") as log:
        result = subprocess.run(
            command, stdout=log, stderr=subprocess.STDOUT, timeout=2400
        )
    results.append(
        {"command": command, "rc": result.returncode, "log": f"{index:02d}.log"}
    )
...
run(
    cook
    + ["verilator-testharness-comp", "-t", target, "--trace-mode", trace, "--quiet"]
)
if suite == "compile":
    raise SystemExit(0)
march = "rv32imc_zicsr_zba_zbb_zbs_zbc" + ("_zifencei" if suite == "pmp" else "")
testlist = (
    "verif/tests/base_pmp.yaml" if suite == "pmp" else "verif/tests/base_rv32_p.yaml"
)
tests = (
    [
        "decreasing_entries_test",
        "exact_csrr_test",
        "granularity_test",
        "locked_outside_tor_test",
        "lsu_tor_test",
    ]
    if suite == "pmp"
    else ["rv32ui-p-add"]
    ...
    for iss in (False, True):
        run(
            ...
            + [
                ...
                "--trace-mode",
                trace,
                "--iss-enabled" if iss else "--no-iss-enabled",
                "--quiet",
            ]
        )
        simulation = (
            Path("build") / target / "simulation/sim_rtl_verilator_testharness" / name
        )
        ...
        if trace != "notrace":
            ...
            assert waveforms and all(
                path.stat().st_size > 100 for path in waveforms
            ), waveforms
        shutil.copytree(
            simulation,
            out / f"{name}-iss-{iss}",
            ignore=shutil.ignore_patterns("*.vcd", "*.fst", "*.dasm"),
        )
