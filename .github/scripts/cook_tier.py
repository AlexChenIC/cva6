...
def checked_summary(
    ...
    if data.get("iss_enabled") is not False:
        raise ValueError("Testlist summary mismatch: iss_enabled")
    cases = data.get("cases")
    if (
        not isinstance(cases, list)
        or not all(isinstance(case, dict) for case in cases)
        or [c.get("test_name") for c in cases] != expected
    ):
    ...
    try:
        ...
        try:
            ...
            if not isinstance(environment, dict):
                raise ValueError("Missing environment metadata mapping")
            evidence["environment"] = environment
            shutil.copy2(metadata, output / "toolchain-environment.yml")
        except (OSError, KeyError, ValueError, yaml.YAMLError) as error:
            evidence["environment_error"] = str(error)
            evidence["status"], rc = "FAIL", 1
            print(f"ERROR: environment metadata: {error}", file=sys.stderr)
