...
def is_valid_matrix_job(job: dict) -> bool:
    ...
    return bool(config and testcase and "${{" not in config and "${{" not in testcase)
...
def build_chart_data(all_data: dict[str, list[dict]]) -> dict:
    ...
    for workflow in WORKFLOW_INFO:
        ...
        durations = [round(run.get("duration_seconds", 0) / 60, 1) for run in runs]
        chart_data[workflow["key"]] = {
            ...
            "count": len(runs),
            "summary": (
                f"{len(runs)} completed runs. Latest job pass rate "
                f"{pass_rates[-1] if pass_rates[-1] is not None else 'unavailable'} percent and duration "
...
def build_workflows_context(
    ...
    for definition in WORKFLOW_INFO:
        runs = [enrich_run(run, now) for run in all_data.get(definition["key"], [])]
        latest = (
            runs[0]
            if runs
            else {
...
def source_relation(workflows: list[dict], thales: dict) -> dict[str, str]:
    ...
    github_shas = {
        workflow["latest"].get("tested_source_sha", "") for workflow in workflows
    }
    github_shas.discard("")
    if (
        not thales_sha
        or not github_shas
        or any(not w["latest"].get("tested_source_sha") for w in workflows)
    ):
...
def main() -> None:
    ...
    context = {
        ...
        "collection": load_json(data_dir / "metadata.json", {}),
    }
