...
This CI provides public GitHub-hosted validation for an initial, known-green
RV32 AXI subset of `master_candidate`. It reuses the target and testlist
contract from the Thales GitLab CI while running an open-source backend.
...
| Tier 2 | `cv32a60x_axi` | `base_rv32_p`, `base_zcmt` | 9 |
| Tier 2 | `cv32a65x_axi` | `base_rv32_p`, `base_zcmt`, `base_pmp` | 14 |

Tier 1 is the 10-test fast subset. Tier 2 is a strict superset and contains
all 23 enabled target/test pairs present in `.testlist_matrix_target` for these
two targets. Disabled testlist entries (`iterations: 0`) are not counted.
...
- Tier 1 runs for pull requests targeting `master_candidate`, direct updates
  to `master_candidate`, and manual dispatch.
- Tier 2 runs after direct updates to `master_candidate` and by manual
  dispatch. It is not duplicated on every pull-request revision.
...
counts, run the local checks, and prevalidate on a fork. Do not mark a failing
job optional merely to make the workflow green; record and triage backend or
functional differences explicitly.
