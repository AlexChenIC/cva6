...
"""Dispatch already compiled TestHarness tests, using Cook's report format."""
    ...
    for name in names:
        ...
        try:
            ...
            if not isinstance(result, dict) or any(
                ...
                raise ValueError("Missing or inconsistent successful run result")
            case.update(status="PASS", detail=str(result.get("detail", "Completed")))
        except typer.Exit as error:
            case["detail"] = f"Run recipe exited with code {error.exit_code}"
...
def testharness_run_testlist(
    simulator: Simulator = typer.Option(
        ..., "--simulator", help="TestHarness simulator"
    ),
    ...
    iss_enabled: bool = typer.Option(False, help="Enable standalone Spike comparison"),
